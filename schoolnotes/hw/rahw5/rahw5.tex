...
\smallbreak
\begin{IEEEproof}
	Suppose \((a_n)_{n=1}^{\infty}\) is Cauchy, and \((b_n)_{n=1}^{\infty}\) is an equivalent sequence. It follows that
	\begin{equation*}
...
\begin{IEEEproof}
	...
	\begin{IEEEeqnarray}{l}
		i,j>N\Rightarrow\abs{a_i-a_j}<1,\Rightarrow \abs{a_j}<1+\abs{a_i}\label{eqn1}\\
		i,j>M\Rightarrow\abs{b_i-b_j}<1,\Rightarrow \abs{a_j}<1+\abs{b_i}\label{eqn2}
	\end{IEEEeqnarray}
	...
	\begin{IEEEeqnarray*}{rCl}
		i>C\Rightarrow\abs{a_ib_i-a'_ib_i}&<&\frac{(1+\abs{b_k})\epsilon}{1+\abs{b_k}}\\
		&=&\epsilon
	\end{IEEEeqnarray*}
