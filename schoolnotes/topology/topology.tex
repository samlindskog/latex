...
\begin{thm}
	Suppose that \(d\) is a metric on \(X=X_1\times\ldots\times X_n\) that satisfies property \(4.4\) defined in Gamelin "Introduction to Topology". Then the open sets in \((X,d)\) are the unions of product sets of the form \(U_1\times\ldots\times U_n\), where \(U_j\) is an open subset of \(X_j,\;1\leq j\leq n\).
\end{thm}
\section{Compactness}
\begin{defi}[Cover]
